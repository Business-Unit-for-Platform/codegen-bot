...
```text
codegen-bot：先按业务 SQL / 数据模型生成业务模块
Clone-ruoyi-vue-pro-Bot：再把生成结果纳入 future layout / 平台底座结构，完成 restructure
```

业务上先运行 codegen，再运行 Clone-Bot/restructure：codegen 负责把业务模型变成模块代码，Clone-Bot 的目标是把代码整理进标准 RuoYi/Yudao 平台结构，而不是在业务模块生成前先“造房子”。
...
GITHUB_OWNER=Business-Unit-for-Platform \
BACKEND_REPO=future-vue-pro \
FRONTEND_REPO=future-ui-admin-vue3 \
bash tools/codegen/publish.sh
```
...
默认布局。Future 平台布局，用于对接 Clone-Bot/restructure 后的标准项目结构：
